<?php 

function api_usuario_post($request) {

    $email = sanitize_email($request['email']);
    $nome = sanitize_text_field($request['nome']);
    $senha = $request['senha'];
    $cep = sanitize_text_field($request['cep']);
    $rua = sanitize_text_field($request['rua']);
    $numero = sanitize_text_field($request['numero']);
    $cidade = sanitize_text_field($request['cidade']);
    $bairro = sanitize_text_field($request['bairro']);
    $estado = sanitize_text_field($request['estado']);

    $user_exists = username_exists($email);
    $email_exists = email_exists($email);

    if(!$email) {
        $response = new WP_Error('email', 'Email é obrigatório.', array('status' => 403));
        return rest_ensure_response($response);
    }

    if(!$senha) {
        $response = new WP_Error('senha', 'Senha é obrigatório.', array('status' => 403));
        return rest_ensure_response($response);
    }

    if(!$nome) {
        $response = new WP_Error('nome', 'Nome é obrigatório.', array('status' => 403));
        return rest_ensure_response($response);
    }

    if(!$user_exists && !$email_exists && $email && $senha && $nome) {
        $user_id = wp_create_user($email, $senha, $email);

        $response = array(
            'ID' => $user_id,
            'display_name' => $nome,
            'first_name' => $nome,
            'role' => 'subscriber',
        );

        wp_update_user($response);

        update_user_meta($user_id,'cep', $cep);
        update_user_meta($user_id,'rua', $rua);
        update_user_meta($user_id,'numero',$numero);
        update_user_meta($user_id,'cidade',$cidade);
        update_user_meta($user_id,'bairro',$bairro);
        update_user_meta($user_id,'estado',$estado);
    } else {
        $response = new WP_Error('email', 'Email já cadastrado.', array('status' => 403));
    }

    return rest_ensure_response($response);
}

function registrar_api_usuario_post() {
    register_rest_route('api', '/usuario', 
        array(
            array(
                'methods' => WP_REST_Server::CREATABLE,
                'callback' => 'api_usuario_post',
            ),
        )
    );
}

add_action('rest_api_init', 'registrar_api_usuario_post');

?>